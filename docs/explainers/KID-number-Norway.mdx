...
Number of digits — 3 to 25 digits where the last digit is the check digit. The KID number must contain only digits ( use numbers from 0-9) and should not be alphanumeric (Agreed upon with the bank)
...
Number of digits allowed in the reference number: Merchant enters into an agreement with the bank to decide the number of digits in the reference number. Chargebee validates if the number (including the check digit) lies between 3 and 25 and if it does not then we throw an error.


Check digit calculation algorithm: Two options — Mod 10 algorithm or Mod 11 algorithm (Mod 10 calculator).

Next number: A random input number that will be used as the starting number to generate the reference number. ( This is obvious because how would you create KID if you don't have something to begin with.
